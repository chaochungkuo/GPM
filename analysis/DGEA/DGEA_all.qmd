...
# Run DESeq2 Analysis
files <- file.path(config$salmon_dir, config$samplesheet$sample, "quant.sf")
names(files) <- config$samplesheet$sample
txi <- tximport(
  files,
  type = "salmon",
...
if (config$length_correction) {
  design_formula <- if (config$paired) ~ batch + group else ~group
  dds <- DESeqDataSetFromTximport(
    txi,
    colData = config$samplesheet,
    design = config$design_formula
  )
} else {
  dds <- DESeqDataSetFromMatrix(round(txi$counts), config$samplesheet, ~group)
}
...
pca_data$group <- config$samplesheet$group # Add group information from samplesheet
...
writeData(wb, "Samplesheets", config$samplesheet)
...
```{r all Samplesheet,  echo=FALSE, results='asis', warning=FALSE, message=FALSE}
datatable(
  config$samplesheet,
  filter = 'top',
