...
`r if(params$go){"# GO Analysis"}`

```{r, echo=FALSE, results="markup", warning=FALSE, message=FALSE, eval=params$go}
...
# Visualize GO results (Dot plot for publication-quality visualization)
dotplot(go_up, title = "GO Enrichment - Upregulated Genes") + theme_minimal()
emapplot(pairwise_termsim(go_up), title = "Enrichment Map - Upregulated Genes")

dotplot(go_down, title = "GO Enrichment - Downregulated Genes") + theme_minimal()
emapplot(pairwise_termsim(go_down), title = "Enrichment Map - Downregulated Genes")
```

`r if(params$gsea){"# GSEA for Whole Expression Profile"}`

```{r, echo=FALSE, results="markup", warning=FALSE, message=FALSE, eval=params$gsea}
...
`r if(params$gsea){"## GSEA Plots {.tabset}"}`
...
for (i in 1:5) {
  if (i <= nrow(gsea_results)) {
    ...
    output <- paste0(
      ...
      "### ", plot_title, "\n\n",
      "```{r echo=FALSE}\n",
      "gseaplot2(gsea_results, geneSetID = ", i, ", title = \"", plot_title, "\")\n",
      "```\n\n"
    )
  }
}
