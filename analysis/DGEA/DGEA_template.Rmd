...
res$color <- ifelse(res$gene_symbol %in% config$highlighted_genes, "orange", 
...
res$label <- ifelse(res$gene_symbol %in% config$highlighted_genes, res$gene_symbol, NA)
...
`r if(go && any(go_up@result$p.adjust < config$pvalueCutoff_GO)){"## GO on Up Genes"}`
...
`r if(go && any(go_down@result$p.adjust < config$pvalueCutoff_GO)){"## GO on Down Genes"}`
