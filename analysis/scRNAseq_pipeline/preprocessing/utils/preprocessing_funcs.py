...
qc_features_fac: Dict[str, List[str]] = {
...
def reduce_outliers(
    adata: AnnData,
    variables: Dict[str, List | Number] | Dict[str, Dict[str, List | Number]],
    subset: bool = False,
) -> pd.Series:
    if subset:
...
def _compute_outlier_all(adata: AnnData, variables: Dict[str, List]) -> pd.Series:
    for key in variables.keys():
        ...
        adata.obs[f"{key}_outlier"] = adata.obs[key].lt(min_val) | adata.obs[key].gt(
            max_val
        )
        adata.obs[f"{key}_outlier"].fillna(False)

    adata.obs["outlier"] = adata.obs[[f"{x}_outlier" for x in variables.keys()]].any(
        axis=1
    )
...
def _compute_outlier_sample(adata: AnnData, variables: Dict[str, List], sample):
    ...
    for key in sample_dict.keys():
        if not key in sample_slice.columns:
            raise KeyError(
                "the provided QC variable does not exist in the data, check the variable names again."
            )

        if not f"{key}_outlier" in sample_slice.columns:
