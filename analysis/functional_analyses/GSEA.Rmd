...
gostres <- run_GSEA(label="ASCs_vs_Pericytes_no_both_outliers")
dotplot(gostres, showCategory=10, split=".sign", font.size = 8, label_format=50) + facet_grid(.~.sign)
table_GSEAres(gostres@result)
```
...
for (i in 1:10) {
  cat("\n### ",  gostres$Description[i], "\n")
  p <- gseaplot(gostres, by = "all", title = gostres$Description[i], geneSetID = i)
