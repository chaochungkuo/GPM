---
title: "Quality Control of scRNAseq"
author:
...
readers_factory = list("10X" = Seurat::Read10X,
...
histogram_seurat = function(obj, variable, n_mad = 3, vlines = FALSE, annotate = FALSE, no_breaks = 30 ){
  ...
  fig = ggplot(obj@meta.data, aes_string(x = variable))+geom_histogram(fill = "#86BAFE", color = "#5DA3FD", bins = no_breaks)+
  theme_classic()
  if(vlines == TRUE){
...
seurat_obj = seurat_obj %>% NormalizeData %>%  FindVariableFeatures %>% ScaleData
seurat_obj = seurat_obj %>% RunPCA(features = VariableFeatures(object = seurat_obj))
seurat_obj = seurat_obj%>% FindNeighbors(dims = 1:20) %>%  FindClusters() %>% RunUMAP(dims = 1:20)
